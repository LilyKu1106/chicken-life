// Arcade v2-fixed-2 — 2026-07-02
// 修正: 等 DOM + UI 都準備好才初始化，避免 closeBtn 為 null 導致整段中斷
(() => {
  'use strict';
  ...
  const init = () => {
    ...
    const list = $('arcade-list');
    const modal = $('arcade-play-modal');
    const box = $('arcade-container');
    const hud = $('arcade-hud');
    const title = $('arcade-title');
    const closeBtn = $('arcade-close');

    // 如果 DOM 還沒長好就重試
    if (!list ||!modal ||!closeBtn) return false;
    ...
    document.addEventListener('click', e => {
      const btn = e.target.closest('[data-action="arcade"]');
      if (btn) {
        SoundManager.click();
        UI.openModal('arcade-modal');
      }
    });

    const games = [
      {n:'🧠 小雞迷宮逃脫',d:'幫牠找到回家的路',p(){title.textContent='迷宮';box.innerHTML='<canvas width=400 height=300></canvas>';const c=box.querySelector('canvas'),x=c.getContext('2d');let px=1,py=1,ex=24,ey=18,t=30;const m=Array(20).fill().map(()=>Array(26).fill(0));for(let i=0;i<200;i++)m[rand(1,18)][rand(1,24)]=1;const draw=()=>{x.fillStyle='#ffe9b8';x.fillRect(0,0,400,300);for(let y=0;y<20;y++)for(let z=0;z<26;z++)if(m[y][z]){x.fillStyle='#5c3b26';x.fillRect(z*15,y*15,15,15);}x.fillStyle='#6fae5a';x.fillRect(ex*15,ey*15,15,15);x.fillStyle='#ffd23f';x.beginPath();x.arc(px*15+7,py*15+7,6,0,7);x.fill();};const k=e=>{let nx=px,ny=py;if(e.key==='ArrowUp')ny--;if(e.key==='ArrowDown')ny++;if(e.key==='ArrowLeft')nx--;if(e.key==='ArrowRight')nx++;if(nx>=0&&ny>=0&&nx<26&&ny<20&&!m[ny][nx]){px=nx;py=ny;}if(px===ex&&py===ey)end(1);};addListener(document,'keydown',k);addTimer(()=>setInterval(()=>{t--;hud.textContent='時間:'+t;if(t<=0)end(0);},1000));function loop(){draw();loopId=requestAnimationFrame(loop);}loop();function end(w){clearAll();if(w){reward({gold:40,happy:15});UI.toast('找到出口！');}else{reward({mood:-10});UI.toast('迷路了');}setTimeout(()=>closeBtn.click(),800);}}},
      {n:'🐛 抓蟲大作戰',d:'抓對食物',p(){title.textContent='抓蟲';box.innerHTML='<canvas width=400 height=300></canvas>';const c=box.querySelector('canvas'),x=c.getContext('2d');let b=[],s=0,t=20;const sp=()=>b.push({x:rand(20,380),y:rand(20,280),t:Math.random()<.7?'g':'b'});const cl=e=>{const r=c.getBoundingClientRect(),mx=e.clientX-r.left,my=e.clientY-r.top;b=b.filter(o=>{if(Math.hypot(o.x-mx,o.y-my)<18){if(o.t==='g'){s+=10;reward({gold:2,hunger:3});}else{reward({mood:-5});}return false}return true});};addListener(c,'click',cl);addTimer(()=>setInterval(sp,600));addTimer(()=>setInterval(()=>{t--;hud.textContent='分數:'+s+' 時間:'+t;if(t<=0)end();},1000));function loop(){x.fillStyle='#fff3d6';x.fillRect(0,0,400,300);b.forEach(o=>{o.y+=1.5;x.font='20px serif';x.fillText(o.t==='g'?'🐛':'☠️',o.x,o.y);});loopId=requestAnimationFrame(loop);}loop();function end(){clearAll();setTimeout(()=>closeBtn.click(),600);}}},
      {n:'⚖ 平衡天秤',d:'讓天秤平衡',p(){title.textContent='平衡';box.innerHTML='<div style="display:flex;justify-content:space-around;padding:20px;"><div id=l style="width:45%;height:100px;background:#8a5a3b"></div><div id=r style="width:45%;height:100px;background:#8a5a3b"></div></div><div id=fb style="text-align:center"></div>';let L=0,R=0;['🌽','🍗','🥚','🍞'].forEach((e,i)=>{const b=document.createElement('button');b.textContent=e;b.style.fontSize='24px';b.onclick=()=>{const w=[3,5,2,4][i];if(L<=R){L+=w;document.getElementById('l').innerHTML+=e;}else{R+=w;document.getElementById('r').innerHTML+=e;}hud.textContent='L:'+L+' R:'+R;if(Math.abs(L-R)<=1&&L+R>=12){reward({gold:30,happy:10});UI.toast('成功');setTimeout(()=>closeBtn.click(),800);}};document.getElementById('fb').appendChild(b);});}},
      {n:'🐔 小雞賽跑',d:'避開障礙',p(){title.textContent='賽跑';box.innerHTML='<canvas width=400 height=300></canvas>';const c=box.querySelector('canvas'),x=c.getContext('2d');let y=200,vy=0,obs=[],d=0,alive=1;const j=()=>{if(y>=200){vy=-10}};addListener(c,'click',j);addListener(document,'keydown',e=>{if(e.code==='Space')j()});function loop(){if(!alive)return;x.fillStyle='#b6e3a1';x.fillRect(0,0,400,300);x.fillStyle='#8a5a3b';x.fillRect(0,230,400,70);y+=vy;vy+=0.6;if(y>200){y=200;vy=0}if(Math.random()<.03)obs.push({x:400});obs.forEach(o=>{o.x-=4;x.fillStyle='#5c3b26';x.fillRect(o.x,210,20,20);if(o.x<60&&o.x>30&&y>180)alive=0});obs=obs.filter(o=>o.x>-20);x.font='30px serif';x.fillText('🐥',40,y+10);d++;hud.textContent='距離:'+d;if(!alive){reward({gold:Math.floor(d/10)});clearAll();setTimeout(()=>closeBtn.click(),800);return}loopId=requestAnimationFrame(loop)}loop()}},
      {n:'🍳 廚房快手',d:'完成料理',p(){title.textContent='廚房';let i=0,s=['加蛋🥚','加玉米🌽','攪拌🥄','完成🍳'];box.innerHTML='<div id=st style="color:#fff;text-align:center;padding:60px;font-size:24px"></div>';const nx=()=>{if(i>=s.length){reward({gold:50});UI.toast('成功');setTimeout(()=>closeBtn.click(),600);return}document.getElementById('st').textContent=s[i];hud.textContent=(i+1)+'/4'};addListener(box,'click',()=>{i++;nx()});nx()}},
      ...
      {n:'🧩 拼圖修復',d:'3x3拼圖',p(){title.textContent='拼圖';box.innerHTML='<div id=pz style="display:grid;grid-template-columns:repeat(3,80px);gap:4px;justify-content:center;padding:20px"></div>';let a=[1,2,3,4,5,6,7,8,0].sort(()=>Math.random()-.5),m=0;const dr=()=>{const pz=document.getElementById('pz');pz.innerHTML='';a.forEach(n=>{const d=document.createElement('div');d.style.cssText='width:80px;height:80px;background:#ffe9b8;display:flex;align-items:center;justify-content:center;font-size:30px';d.textContent=n?'🐥':'';d.onclick=()=>{const i=a.indexOf(n),z=a.indexOf(0);if(Math.abs(i-z)===1||Math.abs(i-z)===3){[a[i],a[z]]=[a[z],a[i]];m++;dr();if(a.join(',')==='1,2,3,4,5,6,7,8,0'){reward({gold:40,happy:10});setTimeout(()=>closeBtn.click(),600)}}};pz.appendChild(d)});hud.textContent='步數:'+m};dr()}},
      {n:'🎲 幸運轉盤',d:'試手氣',p(){title.textContent='轉盤';box.innerHTML='<div style="text-align:center;padding:40px"><button id=sp style="font-size:24px;padding:20px">轉！</button><div id=rs style="color:#fff;margin-top:20px"></div></div>';document.getElementById('sp').onclick=()=>{const p=[{t:'+30金',g:30},{t:'+飼料',f:1},{t:'-5心情',m:-5},{t:'+50金',g:50}][rand(0,3)];document.getElementById('rs').textContent=p.t;if(p.g)reward({gold:p.g});if(p.f)GameState.inventory.food_basic=(GameState.inventory.food_basic||0)+1;if(p.m)reward({mood:p.m});setTimeout(()=>closeBtn.click(),1200)}}},
    ...
    function build(){
      ...
      games.forEach(g=>{
        ...
        it.innerHTML=`<div class="shop-item-info"><span style="font-size:20px">${g.n.split(' ')[0]}</span><div><div style="font-size:10px">${g.n}</div><div style="font-size:8px;color:#5c3b26">${g.d}</div></div></div><button>玩</button>`;
        it.onclick=()=>{ UI.closeModal('arcade-modal'); modal.classList.remove('hidden'); clearAll(); g.p() };
        list.appendChild(it);
      });
    }
  ...
  // 等 DOM + UI 都好
  waitFor(() => document.readyState!== 'loading' && window.UI && window.GameState && window.SoundManager, () => {
    if (!init()) {
      // 如果第一次抓不到，再等一下
      setTimeout(() => init(), 200);
    }
