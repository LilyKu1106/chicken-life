...
const MiniGameSystem = (() => {
  ...
  function launch(id){
    ...
    // 凍結主計時器與獨立排程器：進入小遊戲的瞬間就凍結，離開/結算時才恢復
  ...
  function hide(){
    ...
    // 2. 強制隱藏結算視窗（原本只靠 overlay hidden 不夠，
    //    這個 div 若沒有明確 add('hidden')，依然佔據點擊判定區域）
    if (resultDiv){
      resultDiv.classList.add('hidden');
    }
...
/* ============================================================================
   小遊戲共用工具暴露到 window
   ----------------------------------------------------------------------------
   9 款小遊戲已拆分到 minigames/ 資料夾下的獨立檔案（各自用
   window.XxxGame = (() => {...})() 宣告）。它們執行在全域作用域，不在這個
   IIFE 內部，因此需要用到的核心工具/物件（繪圖函式、亂數工具、GameState、
   SoundManager、UI）必須明確掛到 window 上才能被那些檔案存取。
   這個區塊必須放在所有相關 const 宣告「之後」，且要在小遊戲檔案的
   <script> 標籤「之前」載入（見 index.html 的載入順序）。
   ============================================================================ */
...
window.SLEEP_ENERGY_GAIN = SLEEP_ENERGY_GAIN;
window.clamp             = clamp;
window.rand               = rand;
window.randInt            = randInt;
window.choice             = choice;
window.px                 = px;
window.pxRect             = pxRect;
window.fillPixelCircle    = fillPixelCircle;
window.drawChick          = drawChick;
