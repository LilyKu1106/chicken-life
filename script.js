...
const GameState = {
  ...
  sleepToggle(){
    ...
    if (this.isSleeping){ SoundManager.sleep(); UI.playOneShot('sleep', null); }
...
const UI = {
  ...
  init(){
    ...
    animManager.register('chick', {
      ...
      draw: (frame) => {
        ...
        drawChick(chickCtx, {
          state: GameState.currentDominantState(),
          frame,
  ...
  /** 暫時切換動畫狀態幾百毫秒（例如吃東西、開心一下），結束後回到自然狀態 */
  playOneShot(stateName, ms){
    const actor = animManager.get('chick');
    if (!actor) return;
    const original = GameState.currentDominantState;
    GameState.currentDominantState = () => stateName;
    if (ms){
      setTimeout(() => { GameState.currentDominantState = original; }, ms);
    }
  },
